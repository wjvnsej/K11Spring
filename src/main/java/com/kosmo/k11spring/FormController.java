...
public class FormController {
	...
	*/
	...
	public String loginMember(HttpServletRequest req, Model model) {
		...
		/*
		View 영역에서 전송할 데이터를 Model객체(영역)에 저장한다.
	...
	*/
	...
	조건1 : 쿼리스트링으로 전달되는 파라미터의 갯수와 폼값을
		저장 할 객체(DTO 혹은 VO)의 멤버변수의 갯수가 동일할 때
		사용가능함.
	...
	조건3 : 커맨드객체를 만들 때 파라미터의 갯수는 달라도
		...
		동일해야 하고 이에 해당하는 getter/setter가 생성되어야 한다.
	...
	*/
	...
	/*
	폼값받기 4] @RequestMapping어노테이션으로 폼값 받기
		요청명 ./form 뒤에 붙은 ㄱ밧이 메소드에서 사용가능한 파라미터가
		된다. 아래의 경우 2개의 파라미터를 받아서 사용하게 된다.
		단, 웹 브라우저는 요청명을 경로로 인식하므로 리소스(이미지 등)를
		사용할 때는 경로에 주의해야 한다.
